...
| AI assessment | Google Gemini API (`gemini-2.5-flash`, vision) |
| Backend | Firebase — Firestore, Auth, Cloud Storage |
| Security | Firestore/Storage security rules (role- and ownership-based) |
...
```
lib/
├── main.dart                                # Entry point, auth gate, patient/clinician routing
├── firebase_options.dart                    # Firebase project config (flutterfire configure)
├── secrets.dart                             # Gemini API key (gitignored — see Setup)
├── core/theme/
│   ├── app_colors.dart                      # Brand palette, light/dark aware
│   ├── app_theme.dart                       # Global Material theme
│   └── theme_controller.dart                # Persisted theme mode
├── data/
│   ├── models/
│   │   ├── assessment.dart                  # Assessment model + RiskLevel enum
│   │   ├── patient_profile.dart             # Clinical context + clinician contact
│   │   └── symptom_response.dart            # Symptom questionnaire response
│   └── services/
│       ├── ai_service.dart                  # Gemini API integration
│       ├── auth_service.dart                # Firebase Auth (patient + clinician-created accounts)
│       └── firestore_service.dart           # Firestore reads/writes, FlaggedCase/PatientSummary models
└── features/
    ├── home/home_screen.dart                # Daily check-in card, last assessment, streak
    ├── assessment/                          # Guided capture → symptoms → AI analysis → result
    ├── history/                             # Longitudinal assessment list + entry detail
    ├── notifications/                       # Clinician-review notifications
    ├── profile/                             # Patient profile, care team, FAQ
    ├── auth/                                # Login/register, forced password reset
    ├── clinician/                           # Dashboard, patients, case detail, clinician profile
    └── shared/widgets/                      # RiskBadge, StreakHistory, ZoomableImage, skeletons
```
